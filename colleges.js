(() => {
  ...
  function colleges() {
    ...
    // Remove specific colleges from the cards list only (data remains intact).
    list = list.filter((c) => !EXCLUDED_COLLEGES.has(c.name));
  ...
  function renderGrid(list) {
    ...
    grid.innerHTML = list
      .map((col) => {
        ...
        // Photo and logo are resolved locally from assets/college-photo and assets/college-logo (by city).
        // Keep a neutral fallback gradient until the actual image loads.
        const photo = "";
        const imgCls = `college-img ${bg}`;

        return `
          <div class="college-card" data-col="${escapeHtml(col.name)}">
            <div class="${imgCls}" ${photo} data-city="${escapeHtml(inf.city || "")}">
            ...
            <div class="college-info">
              ...
              <div class="college-stats">
                <div class="stat">
                  ...
                  <div class="stat-label">Avg JEE AIR (CSE)</div>
                </div>
                <div class="stat">
    ...
    // Prefer local logos (by city filename) as an overlay when available.
    grid.querySelectorAll(".college-img[data-city]").forEach((el) => {
      const city = el.getAttribute("data-city") || "";
      if (!city) return;
      applyCollegePhotoBackground(el, city);
      applyCollegeLogoOverlay(el, city);
    });
  }
