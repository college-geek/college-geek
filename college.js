(() => {
  const { DATA, fmt, fmtR, sh, getTier, cColor, escapeHtml, renderNav, renderFooter, applyCollegeLogoOverlay, applyCollegePhotoBackground } =
    window.CG;
  ...
  function renderHeader() {
    ...
    dPills.innerHTML = `
      <span class="meta-pill">📊 Avg JEE AIR (CSE): <b>${escapeHtml(fmt(m.avg_air))}</b></span>
      <span class="meta-pill">🎓 Branches: <b>${escapeHtml((m.branches || []).length)}</b></span>
    `;

    // Photo/logo are resolved locally from assets/college-photo and assets/college-logo (by city).
    dPhoto.classList.remove("has-photo");
    dPhoto.style.backgroundImage = "";
    dPhoto.innerHTML = "<span>🏫</span>";

    // Prefer local logo (by city filename) as an overlay when available.
    if (inf.city) {
      applyCollegePhotoBackground(dPhoto, inf.city);
      applyCollegeLogoOverlay(dPhoto, inf.city);
    }
  }
  ...
  function renderBody() {
    ...
    // Transport tab removed from UI; if the URL/state still has it, fall back to About.
    if (state.detTab === "transport") state.detTab = "about";
        ...
        <div class="info-grid" style="margin-top:12px">
          <div class="info-card">
            ...
            <div style="color:var(--text-secondary);line-height:1.7">
              📊 Avg JEE AIR (CSE): <b style="color:var(--text)">${escapeHtml(fmt(m.avg_air))}</b><br/>
    ...
    if (state.detTab === "fees") {
      const feeItems = [
        { label: "Tuition Fee", amount: 3200, suffix: "/yr" },
        { label: "Examination Fee", amount: 7400, suffix: "/yr" },
        { label: "Hostel Fee", amount: 14400, suffix: "/yr" },
        { label: "Mess Fee", amount: 40000, suffix: "/yr" },
        { label: "Other Fee", amount: 2000, suffix: "" },
      ];
      const total = feeItems.reduce((s, x) => s + (Number(x.amount) || 0), 0);

      dBody.innerHTML = `
        <div class="info-card" style="border-radius:16px">
          <div class="k">Hostel & Fees</div>
          <div class="info-grid" style="margin-top:12px">
            <div class="info-card" style="border-radius:14px">
              <div class="k">Hostel Availability</div>
              ${
                inf.hostel
                  ? `
                  ...
                  `
                  : `<div class="v" style="color:var(--text-secondary)">No hostel availability details available.</div>`
              }
            </div>
          </div>
          <div class="k" style="margin-top:14px">Annual Fee Structure</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:8px">
            ${feeItems
              .map(
                (x) => `
                <div style="border:1px solid var(--border);border-radius:14px;padding:12px;background:var(--bg-gray)">
                  <div style="font-size:.75rem;color:var(--text-secondary);font-weight:700;margin-bottom:6px">${escapeHtml(x.label)}</div>
                  <div style="font-weight:900">₹${escapeHtml((Number(x.amount) || 0).toLocaleString("en-IN"))}${escapeHtml(x.suffix || "")}</div>
                </div>
              `
              )
              .join("")}
          </div>
          <div style="margin-top:12px;color:var(--text-secondary);line-height:1.7">
            <div>All fees are approximate.</div>
            <div>Fee waiver available for SC/ST students.</div>
          </div>
          <div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--border);display:flex;justify-content:space-between;align-items:center">
            <span style="font-weight:900">Total Annual Cost</span>
            <span style="font-weight:900;color:var(--blue)">₹${escapeHtml(total.toLocaleString("en-IN"))} <span style="font-size:.85rem;color:var(--text-secondary)">(approx.)</span></span>
          </div>
        </div>
      `;
      return;
